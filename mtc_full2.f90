...
REAL, PARAMETER :: h7      = 5.5e3              ! [M] Height at 266MB
REAL, PARAMETER :: hs0     = 60.               ! [M] Ocean mixed layer deep
REAL, PARAMETER :: hc      = 0.!2.5            ! [M] continent deep
REAL, PARAMETER :: kaust   = 10e6             ! [K S^-1] Austausch coeficient
...
REAL, PARAMETER :: tm0     = t0 + gamma*h0/2. + 20.
...
REAL, DIMENSION(nlat)           :: lat, fcor, phi, erre       ! latitude vector
...
REAL, DIMENSION(nlon,nlat)      :: f84, f85, f87a, f88a, f89a, f90a, tpm, senp, cosp
...
REAL, DIMENSION(nlon,nlat)      :: tm1, g5a, bp1, cp1, dp1, g5dnm
    ...
    DO ext = 1 , nsim
        ...
        DO icase = 1 , 2
            ...
            DO m = 1 , nlon*nlat
                ...
                f38    = g3n(m) !- cont*casedn*(1.-d7(m))*esn(m)
                f41    = ocean*k4b*van(m)*0.981*casedn                        !V
                f42    = - hum(m)/0.981*f41 +                                  &
                         casedn*cont*(1.-d7(m))*k2*van(m)*(1.-hum(m))         !V
                f43    = 0.!cont*( 1 - d7(m) )*casedn                         !V
                f44    = g2n(m) + cont*(1.-g2p)*d8(m)                         !V
                f46    = van(m)*(ocean*k3+cont*k2*g2p)*casedn                 !V
                f47    = cont*(1.-g2p)*d7(m)                                  !V
                f48    = f34 + f34p*cld(m) + a1(m)                            !V
                f49    = - qq0(m)*(1.-xk(m))*(1.-albs(m))                     !V
                f68    = 0.!bp(m)*casedn/20.                       !!!!!
                f68p   = 0.!ap(m)*casedn/1000                         !!!!!
...
!-------------------------------------------------------------------------------
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
!                   ---------------------------- CHGTS
                    pn(i,j)   = tmpn(m) - gamma*(h7 - h700(j)) + pcdn(m)*casedn
                    tn(i,j)   = tmpn(m) - gamma*(h0 - h700(j))
                    tmna(i,j)  = tmn(m)
!                   ----------------------------
                    m         = m + 1
                ENDDO
            ...
            CALL deriv_sph_ct(nlon,nlat,lon,lat,tmna,dtndx,dtndy)
            m = 1
            DO j = 1 , nlat
                DO i = 1 , nlon
                    phi(j)    = -lat(j)*pi/180.
                    fcor(j)   = 2*om*SIN(phi(j))
                    f0        = ( fcor(j)+erre(j) ) / ( fcor(j)**2+erre(j)**2 )
                    r0        = ( fcor(j)-erre(j) ) / ( fcor(j)**2+erre(j)**2 )
                    f8g       = -cv*f0*t0/(gamma*(grb-1))*(1-(ta0/t0)**(grb+1))
                    f8r       = -cv*r0*t0/(gamma*(grb-1))*(1-(ta0/t0)**(grb+1))
                    f8pg      = p0/t0*(1-grb)*(f8g+f8r) - g/gamma * (f0+r0)*f82
                    f8pr      = p0/t0*(1-grb)*(f8g-f8r) - g/gamma * (f0-r0)*f82
                    f84(i,j)  = f8g*dpdy(i,j)   !- f8r*dpdy         + &
                    !            !f8ppg*dtdy(i,j) + f8ppr*dtdy(i,j)  + &
                    !            !f8pg*dtdy(i,j)  + f8pr*dtdy(i,j)
                    f85(i,j)  = f8g*dpdx(i,j) !- f8r*dpdx
                    f90a(i,j) = f90(m) + f79(m)*f8pg*( &
                                dtdx(i,j)*dtndy(i,j)- dtdy(i,j)*dtndx(i,j) ) + &
                                f79(m)*f8pr*( &
                                dtdx(i,j)*dtndy(i,j)- dtdy(i,j)*dtndx(i,j) )
                ENDDO
            ENDDO
!-------------------------------------------------------------------------------
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    geo(i,j) = rd*(t0+gamma*h0/2)/t0 * tmpn(m) - &
                               g*h0/(2*t0) * tmpn(m)
                    m        = m + 1
                    ENDDO
                ENDDO
            CALL deriv_sph_ct(nlon,nlat,lon,lat,geo,dgdl,dgdf)
            m = 1
            DO j = 1 , nlat
                DO i = 1 , nlon
                    phi(j)    = -lat(j)*pi/180.
                    fcor(j)   = 2*om*SIN(phi(j))
                    erre(j)   = 1e-5!1/lat(j)*(1e-3)*(phi(j))
                    senp(i,j) = SIN(phi(j))
                    cosp(i,j) = COS(phi(j))
                    um(m)     = +( fcor(j) / (fcor(j)**2+erre(j)**2) ) * dgdf(i,j) &
                                +( erre(j) / (fcor(j)**2+erre(j)**2) ) * dgdl(i,j)
                    vm(m)     = ( fcor(j) / ( fcor(j)**2 + erre(j)**2 ) ) * dgdl(i,j) &
                                -( erre(j) / ( fcor(j)**2 + erre(j)**2 ) ) * dgdf(i,j)
                    um(m)     = um(m)*1.5
                    !vm(m)     =
                    umdn(m)   = um(m) - umn(m)
                    vmdn(m)   = vm(m) - vmn(m)
                    f87a(i,j) = -umn(m) - umdn(m)*casedn
                    f88a(i,j) = -vmn(m) - vmdn(m)*casedn - kaust*senp(i,j)/cosp(i,j)/r
                    f89a(i,j) = f89(m)
                    f90a(i,j) = f90(m)
                    m         = m + 1
                ENDDO
            ...
            ELSE
                ...
                DO j = 1 , nlat
                    DO i = 1 , nlon
                        ...
                        !aux2(i,j,ext) = f87a(i,j)
                        aux2(i,j,ext) = um(m)
                        !write(*,*) aux2(i,j,ext)*1e10
                        m=m+1
                    ENDDO
                ENDDO
            ENDIF
