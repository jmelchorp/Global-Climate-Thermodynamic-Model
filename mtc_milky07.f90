...
! mtc_global.f90                                                               =
!                                                                              =
!  Model version in which wind anomalies computed after the first loop of      =
!  normal/abnormal cases (N & DN). In The first loop, N & DN temperatures are  =
!  computed using climatological values of zonal and meridional winds, and     =
!  this temperatures N & DN, the wind anomalies and the pressure anomalies are =
!  calculated.                                                                 =
!                                                                              =
!===============================================================================
! FLAGS SWITCH:
...
!===============================================================================
INTEGER,PARAMETER :: cldera = 0
INTEGER,PARAMETER :: milan  = 1
INTEGER,PARAMETER :: g22p   = 1
INTEGER,PARAMETER :: lh     = 1
INTEGER,PARAMETER :: xk0    = 1
INTEGER,PARAMETER :: a20    = 1
INTEGER,PARAMETER :: a30    = 1
!============================ Advection parameters =============================
!------------------------------------------------------------------*
! OPTION *         * dv1 * dv2 * dv3 * dv4 * dv5 * dv7 * dv8 * dv9 *
!--------*---------*-----*-----*-----*-----*-----*-----*-----*-----*
...
!        *   AB    *  0  *  0  *  0  *  0  *  0  *  0  *  0  *  1  *
!--------*---------*-----*-----*-----*-----*-----*-----*-----*-----*
REAL, PARAMETER :: dv1 = 0.
REAL, PARAMETER :: dv2 = 0.
REAL, PARAMETER :: dv3 = 0.
REAL, PARAMETER :: dv4 = 0.
REAL, PARAMETER :: dv5 = 0.
REAL, PARAMETER :: dv6 = 0.
REAL, PARAMETER :: dv7 = 0.
REAL, PARAMETER :: dv8 = 0.
REAL, PARAMETER :: dv9 = 0.
...
INTEGER, PARAMETER       :: nsim    = 6                   ! forecast extension
INTEGER, PARAMETER       :: dateini = 197212               ! first forecast date
INTEGER, PARAMETER       :: dateend = 197212               ! last  forecast date
...
REAL :: tt, tlower, tupper, tasurf, plower, rholow, pr
...
REAL, DIMENSION(nlon*nlat)      :: van, vmn, umn , umdn, vmdn
REAL, DIMENSION(nlon*nlat)      :: f72p, f73p, f75p, f76p, f77p, f79
REAL, DIMENSION(nlon*nlat)      :: f89, f90
REAL, DIMENSION(nlon,nlat)      :: f87, f88, f87a, f88a, f89a, f90a
REAL, DIMENSION(nlon,nlat)      :: tpm, tn, tpdn, senp, cosp!, adn
REAL, DIMENSION(nlon,nlat)      :: pp, pn, pc, pcn, pcdn, pscdn
REAL, DIMENSION(nlon,nlat)      :: vxdn, vydn, vxn, vyn, vx, vy
REAL, DIMENSION(nlon,nlat)      :: hup, h500a, h500n, h500p, h5cdn
!REAL, DIMENSION(nlon,nlat)      :: dhdx, dhdy
REAL, DIMENSION(nlon,nlat)      :: dpdx, dpdy, dpcdx, dpcdy, dpdndx, dpdndy
REAL, DIMENSION(nlon,nlat)      :: dtdx, dtdy, dtndx, dtndy, dtcdx , dtcdy
REAL, DIMENSION(nlon,nlat)      :: dtdndx, dtdndy
REAL, DIMENSION(nlon,nlat)      :: jac_tpm_pn !, jac_tpm_tpn, jac_tpm_tmn
!REAL, DIMENSION(nlon,nlat)      :: jac_tpn_tpm, jac_tpn_tmn, jac_tpn_pn
REAL, DIMENSION(nlon,nlat)      :: jac_tmn_tpn
...
DO n = 1 , ntime
    ...
    reg = (yearini-1948)*12+mesini+n-1
    IF ( reg < 1 ) THEN
    ...
    DO ext = 1 , nsim
        ...
        DO j = 1 , nlat
            DO i = 1 , nlon
                ...
                pn(i,j) = 500*100*(tt/t500(m))**grb !+ pcdn(i,j)*casedn
                tn(i,j) = tpn(m) - t0
                m       = m + 1
            ENDDO
        ENDDO
        ...
        DO icase = 1 , 2
            ...
            CALL jacobin( tm1  , tn+t0, jac_tmn_tpn)
            CALL jacobin( tn+t0, pn   , jac_tpm_pn)
            ! ------------------------------------------------------------------
            m = 1
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    f87a(i,j) = - umn(m) - vxdn(i,j)*casedn + f87(i,j)*casedn
                    f88a(i,j) = - vmn(m) - vydn(i,j)*casedn + f88(i,j)*casedn  &
                                - kaust*senp(i,j)/cosp(i,j)/r
                    f89a(i,j) = f89(m)
                    f90a(i,j) = f90(m) + 1./cosp(i,j)**2/r**2* &
                                         (dv4*b3*jac_tmn_tpn(i,j) +            &
                                         dv5*b2*jac_tpm_pn(i,j) ) /f83
                    m         = m + 1
                ENDDO
            ENDDO
            ! === CALL LAMBDA ==================================================
...
!IF ( itera > 1 ) CALL pressure ( tpn, tpm, tmn, tpdn, h7cdn, pn, pc, pp )
            !IF ( itera > 1 ) THEN
            IF ( icase == 1 ) THEN
                m = 1
                DO j = 1 , nlat
            ...
            ELSE
                ...
                DO j = 1 , nlat
                    DO i = 1 , nlon
                        hup(i,j) = h250(m) + a * ( tpm(i,j) - tm1(i,j) )
                        !m = m + 1
                    ENDDO
                ENDDO
            ENDIF
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    h5cdn(i,j) = h500p(i,j) + h500a(i,j) - h500n(i,j)
                    !m = m + 1
                ENDDO
            ENDDO
            ENDIF
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    tupper     = tt - gamma * ( hup(i,j) - h0 )
                    tasurf     = tt + gamma * h0
                    pc(i,j)    = p0*(tt/tasurf)**grb
                    pscdn(i,j) = pc(i,j)*(tasurf/tt)**grb
                    !m = m + 1
                ENDDO
            ENDDO
            IF ( icase == 1 ) THEN
            ...
            ENDIF
            !ENDIF
!IF ( itera > 1 ) CALL advection ( tpn, tpm, tmn, tpdn, h7cdn, pn,pc,pcdn )
!            IF ( itera > 1 ) THEN
            !CALL deriv_sph_ct(nlon,nlat,lon,lat,h500p,dhdx,dhdy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,pcn,dpdx,dpdy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,tm1,dtdx,dtdy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,pc,dpcdx,dpcdy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,tpm,dtcdx,dtcdy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,pcdn,dpdndx,dpdndy)
            CALL deriv_sph_ct(nlon,nlat,lon,lat,tpdn,dtdndx,dtdndy)
!            CALL jacobin( tpm  , pp   , jac_tpm_pn  )
!            CALL jacobin( tpm  , tn+t0, jac_tpm_tpn )
!            CALL jacobin( tpm  , tm1  , jac_tpm_tmn )
!            CALL jacobin( tn+t0, tpm  , jac_tpn_tpm )
!            CALL jacobin( tn+t0, tm1  , jac_tpn_tmn )
!            CALL jacobin( tn+t0, pp   , jac_tpn_pn  )
!            CALL jacobin( tpm  , pp   , jac_tpm_pn  )
            DO j = 1 , nlat
                DO i = 1 , nlon
                    phi(j)    = -lat(j)*pi/180.
                    ...
                    cosp(i,j) = COS(phi(j))
                    fcor      = 2*om*SIN(phi(j))
                    erre      = 6e-4*EXP(-ABS(phi(j)/(10.*pi/180.)))
                    fg        = fcor / (fcor**2+erre**2)
                    fr        = erre / (fcor**2+erre**2)
!                    adn(i,j)  = 1./cosp(i,j)**2/r**2 * (                       &
!                                dv1*b2*jac_tpm_pn(i,j)             +           &
!                                (dv1*b1 - dv3*b3)*jac_tpm_tpn(i,j) -           &
!                                dv2*b3*jac_tpm_tmn(i,j)*casedn     +           &
!                                dv4*( jac_tpn_tpm(i,j)*casen       +           &
!                                      jac_tpn_tmn(i,j)*casedn )    +           &
!                                dv5*b2*jac_tpn_pn(i,j) + dv9*b2*jac_tpm_pn(i,j))
                    vx(i,j)   = b2*( fg*dpcdy(i,j) + fr*dpcdx(i,j) ) / f83    +  &
                                b3*fg*dtcdy(i,j)/f83 + &
                                b1*fr*dtcdy(i,j)/f83

                    vy(i,j)   = b2*( fg*dpcdx(i,j) - fr*dpcdy(i,j) ) / f83    +  &
                                b3*fg*dtcdx(i,j)/f83 + &
                                b1*fr*dtcdx(i,j)/f83

                    vxn(i,j)  = b2*( fg*dpdy(i,j) + fr*dpdx(i,j) ) / f83    +  &
                                b3*fg*dtdy(i,j)/f83   + &
                                b1*fr*dtdy(i,j)/f83   + &
                                b1*fg*dtdy(i,j)/f83  !+ &
                                !b3*fr*dtdy(i,j)/f83

                    vyn(i,j)  = b2*( fg*dpdx(i,j) - fr*dpdy(i,j) ) / f83    +  &
                                b3*fg*dtdx(i,j)/f83   + &
                                b1*fr*dtdx(i,j)/f83   +  &
                                b1*fg*dtdx(i,j)/f83  !+  &
                                !b3*fr*dtdx(i,j)/f83

                    vxdn(i,j) = vx(i,j) - vxn(i,j)
                    vydn(i,j) = vy(i,j) - vyn(i,j)
                ENDDO
            ENDDO
!            ENDIF
! ------------------------------------------------------------------------------
            ...
            IF ( icase == 1 ) THEN
                ...
                DO j = 1 , nlat
                    DO i = 1 , nlon
                        ...
                        tpmn(i,j,ext) = tmn(m)
                        !tpsn(i,j,ext) = tsn(m)
                        !aux1(i,j,ext) = vxdn(i,j)   ! PRINT OUT NORMAL VARIABLE
                        m      = m + 1
                    ENDDO
                ENDDO
            ELSE
                ...
                DO j = 1 , nlat
                    DO i = 1 , nlon
                        ...
                        aux1(i,j,ext) = vxdn(i,j)   ! PRINTOUT ANOMAL VARIABLE
                        aux2(i,j,ext) = vydn(i,j)
                        m = m + 1
                    ENDDO
                ENDDO
            ENDIF
...
OPEN(UNIT=44,FILE=rutaclim//'TPN.DATA', STATUS='UNKNOWN', ACCESS='DIRECT', &
     FORM='UNFORMATTED',RECL=nlon*nlat*8)
WRITE(*,*) 'Surface Temperature Anomaly (NCEP 1948-2016)'
...
OPEN(unit=69,FILE=rutaclim//'H250.DATA',STATUS='UNKNOWN', ACCESS='DIRECT', &
     FORM='UNFORMATTED',RECL=nlon*nlat*8)
WRITE(*,*) 'Zonal Wind Anomaly (NCEP 1948-2016)'
OPEN(UNIT=70,FILE=rutaclim//'UMDN.DATA',STATUS='UNKNOWN', ACCESS='DIRECT', &
     FORM='UNFORMATTED',RECL=nlon*nlat*8)
WRITE(*,*) 'Meridional Wind Anomaly (NCEP 1948-2016)'
OPEN(UNIT=71,FILE=rutaclim//'VMDN.DATA',STATUS='UNKNOWN', ACCESS='DIRECT', &
     FORM='UNFORMATTED',RECL=nlon*nlat*8)
IF ( xk0 == 1 ) THEN
