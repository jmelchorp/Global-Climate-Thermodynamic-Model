...
REAL, DIMENSION(nlon*nlat)      :: f72, f73, f75, f76, f77
REAL, DIMENSION(nlon*nlat)      :: f89, f90
REAL, DIMENSION(nlon,nlat)      :: f87a, f88a, f89a, f90a, tpm, senp, cosp
...
!                f78    = f32 + cld(m)*f32p + f46 + f47*f36
                f79    = 1. + d2*casedn*( (f49+f34p)*f47 + f30p + a3(m)*sw(m) +&
                         f78*f75(m))                                          !V
                f80    = f37 - f46 + f47*f35 + f78*f73(m) + f79*f68           !V
                f81    = f30 + f44 + f47*f48 + f72(m)*f78 +                    &
                         ...
                         + (f78*f77(m) + f46 -f79*f68 )*tmn(m)*casedn         !V
!                f81    = f30 + f44 + f47*f48 + f72(m)*f78 +                    &
!                         cld(m)*( f30p + a3(m)*sw(m) ) + a2(m)*sw(m) +         &
!                         g5n(m) + ( f78*f76(m) - f46 )*tsn(m)*casedn           &
!                         + (f78*f77(m) + f46 -f79*f68 )*tmn(m)*casedn
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    cosp(i,j) = COS(phi)
                    f87a(i,j) = -um(m)
                    f88a(i,j) = -vm(m) - kaust*senp(i,j)/cosp(i,j)/r
                    f89a(i,j) = f89(m)
                    f90a(i,j) = f90(m)
                    m         = m + 1
                ENDDO
            ...
            DO j = 2 , nlat-1
                DO i = 2 , nlon-1
                    ...
                    cp1(i,j)*( tpm(i-1,j) - tpm(i+1,j) - tm1(i-1,j) + tm1(i+1,j) ) + &
                    dp1(i,j)*( tpm(i,j-1) - tpm(i,j+1) - tm1(i,j-1) + tm1(i,j+1) ) )
                    !write(*,*) g5dnm(i,j)
                ENDDO
            ENDDO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
CALL makedat3d('output/VAR'//labeldate//'.NC','AUX', aux1 , &
               'n/a' , 'n/a' , 'n/a' , 'n/a' , 'n/a',      &
