...
!REAL, DIMENSION(nlon,nlat)      ::
REAL, DIMENSION(nlon,nlat)      :: pn, tn
    ...
    DO ext = 1 , nsim
        ...
        DO j = 1 , nlat
            DO i = 1 , nlon
                ...
                !if (i==100 .and. j==45) write(*,*) pn(i,j)
                tn(i,j) = tpn(m) - t0
                m       = m + 1
            ENDDO
        ENDDO
        ...
        DO icase = 1 , 2
            ...
            ! Se calculan las derivadas espaciales de P y T con diferencias ----
            ! finitas centradas. -----------------------------------------------
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    !f68      = bp(m)*casedn/1e6                   !!!!!!!!!!!!!
                    f68p     = 0.!ap(m)*casedn/1.5e8                    !!!!!!!!!!!!!
                    f69      = cp(m)*casedn/5e-3                    !!!!!!!!!!!!!
                    f70      = dp(m)*casedn/5e-3                    !!!!!!!!!!!!!
                    f70p     = 0.!ep(m)*casedn/1.5e8                    !!!!!!!!!!!!!
                    f84(i,j) =  b2*3e-8*( fg*dpdy(i,j) + fr*dpdx(i,j) ) !+ &
                               !b1*( dtdy(i,j) ) - b3*( dtndy(i,j) ) ) /f83
                    f85(i,j) =  b2*3e-8*( fg*dpdx(i,j) - fr*dpdy(i,j) ) !+ &
                               !b1*( dtdx(i,j) ) - b3*( dtndx(i,j) ) ) /f83
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    if (i==100 .and. j==45) write(*,*) f87(i,j)
                    f89a(i,j) = f89(m)
                    f90a(i,j) = f90(m)
                    m         = m + 1
                ENDDO
...
OPEN(UNIT=44,FILE=rutaclim//'TPN.DATA',STATUS='UNKNOWN', &
     ACCESS='DIRECT',FORM='UNFORMATTED',RECL=nlon*nlat*8)
!
...
OPEN(UNIT=65,FILE=rutaclim//'DP.DATA',STATUS='UNKNOWN', &
     ACCESS='DIRECT',FORM='UNFORMATTED',RECL=nlon*nlat*8)
OPEN(UNIT=66,FILE=rutaclim//'EP.DATA',STATUS='UNKNOWN', &
     ACCESS='DIRECT',FORM='UNFORMATTED',RECL=nlon*nlat*8)
