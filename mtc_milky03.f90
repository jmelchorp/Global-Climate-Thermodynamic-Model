...
REAL, PARAMETER :: eps     = 22.5*pi/180.
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    f84(i,j)  =  b2*( fg*dpdy(i,j) + fr*dpdx(i,j) ) / f83  + &
                                 b3*fg*dtdy(i,j)/f83 + b1*fg*dtdy(i,j)/f83 + &
                                 b3*fg*dtndy(i,j)/f83
                    f85(i,j)  =  b2*( fg*dpdx(i,j) - fr*dpdy(i,j) ) / f83  + &
                                 b3*fg*dtdx(i,j)/f83 + b1*fg*dtdx(i,j)/f83 + &
                                 b3*fg*dtndx(i,j)/f83
            ...
            ELSE
                ...
                DO j = 1 , nlat
                    DO i = 1 , nlon
                        ...
                        aux2(i,j,ext)  = f87(i,j)   ! PRINT OUT ANOMAL VARIABLE
                        m = m + 1
                    ENDDO
                ENDDO
            ENDIF
