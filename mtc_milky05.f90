...
! mtc_global.f90                                                               =
!                                                                              =
!  Model version in which wind anomalies are prescribed.                       =
!                                                                              =
!===============================================================================
! FLAGS SWITCH:
...
!REAL, DIMENSION(nlon,nlat)      :: f84, f85
        ...
        DO icase = 1 , 2
            ...
            DO j = 1 , nlat
                DO i = 1 , nlon
                    ...
                    f87a(i,j) = -umn(m) -umdn(m)*0.25*casedn + f87(i,j)*casedn
                    f88a(i,j) = -vmn(m) -vmdn(m)*0.25*casedn + f87(i,j)*casedn      &
                                - kaust*senp(i,j)/cosp(i,j)/r
                    f89a(i,j) = f89(m)
                    f90a(i,j) = f90(m) + 1./(cosp(i,j)*r**2)*( b3*jac1(i,j) +  &
                                         b2*jac2(i,j) ) /f83
                    m         = m + 1
                ENDDO
            ENDDO
